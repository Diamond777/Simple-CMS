<?
	...
	if (!empty($REQUEST[3]) && @file_exists(dirname(__FILE__).'/inc/'.$REQUEST[2].'.php')) {
		switch ($REQUEST[2]) {
			...
			case 'menu':		$HEADING = ($REQUEST[3] == 'edit') ? 'Редактировать модуль' : 'Добавить модуль'; break;
			case 'item':		$HEADING = ($REQUEST[3] == 'edit') ? 'Редактировать пункты меню' : 'Добавить пункт меню'; break;
			case 'edit_attr':	$HEADING = ($REQUEST[3] == 'edit') ? 'Редактировать пункты меню' : 'Добавить пункт меню'; break;
