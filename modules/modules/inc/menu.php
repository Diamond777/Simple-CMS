<?
	...
	$form->addElement('menu_heading', 'text', 'Название модуля', array(
		'required' => true,
		'style' => 'width:100%;'
	));
	
	$form->addElement('menu_sort', 'text', 'Сортировка', array('style' => 'width:100%;'));
	...
	if ($REQUEST[3] == 'add') {
		...
		elseif ($upload === false) {
			...
			if ($edit) $form->setDataById($edit);
				...
				if (!$edit && @!$base_data['menu_sort']) {
					$max = $db->query("SELECT MAX(`menu_sort`) FROM `".$MODULE_TABLE[$general_table]."`;")->fetchColumn();
					$base_data['menu_sort'] = (floor($max / 10) + 1) * 10;
				}
			}
		}
...
################################################################################
		...
		echo '<div class="form_row">'.$form->getElement('menu_sort').'</div>';
